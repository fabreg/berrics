...
class NewsController extends DailyopsController {
	...
	public function beforeFilter() {
		 ...
		 */
			$date_in = strtotime($this->params['pass'][0]."-".$this->params['pass'][1]."-".$this->params['pass'][2]);
			...
			if($date_in <=1) {
				...
				$date_in = $this->params['pass'][0]."-".$this->params['pass'][1]."-".$this->params['pass'][2];
			...
			if(isset($_GET['date_in'])) {
				...
				$this->params['date_in'] = $_GET['date_in'];
				
			}
			
			if(isset($this->params['named']['date_in'])) {
				
				$this->params['date_in'] = $this->params['named']['date_in'];
	...
	public function section() {
		...
		$token = "news_section_".$this->params['date_in'];
		...
		$older_date = $this->Dailyop->getNextDate($this->params['date_in']);
		
		$newer_date = $this->Dailyop->getNextDate($this->params['date_in'],false);
	...
	public function events_menu() {
		...
		$this->set("event_posts_count",$this->params['paging']['Dailyop']['count']);
