...
<?php echo $this->Form->create('MediaFile');?>
	<fieldset>
 		<legend><?php __('Edit Media File'); ?></legend>
	<?php
		...
		echo "<div>";
			echo $this->Html->link("Preview Video","http://www.theberrics.com/dailyopspost.php?postid=".$this->data['MediaFile']['legacy_entry'],array("target"=>"_blank"));
		echo "</div>";
	?>
	</fieldset>
	<fieldset>
		...
		<div style='padding-top:10px; font-style:italic;'>
			<?php if($this->data['MediaFile']['limelight_mediavault_active']==1): ?>
				<?php echo LLMediaVault::instance()->returnSecureUrl($this->data['MediaFile']); ?>
			<?php else: ?>
