...
<div class='paragraph-bit'>
	...
	<div style='width:19%; float:right;' class='media-file'>
		...
		<?php 
			...
			if(count($paragraph['MediaFile'])>0) {
				...
				echo $this->Html->link("Remove Item",array(),array("rel"=>"remove-media-item","paragraph_id"=>$p['id']));
			}
	...
	<div style='clear:both; font-size:70%;'>
		<?php 
			...
			echo $this->Html->link("Update All Paragraphs","",array("class"=>"update-all-paragraphs"));	
		
		?>
		 | <?php echo $this->Html->link("Add New Paragraph Below","",array("rel"=>"add-paragraph","below"=>$p['sort_weight'])); ?>
		 | <?php echo $this->Html->link("Delete Paragraph","",array("rel"=>"delete-paragraph","paragraph_id"=>$p['id'])); ?> 
		 | <?php echo $this->Html->link("Move Up","",array("rel"=>"move-up","paragraph_id"=>$p['id'])); ?> 
		 | <?php echo $this->Html->link("Move Down","",array(),"",array("rel"=>"move-down","paragraph_id"=>$p['id'])); ?>
