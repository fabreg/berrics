...
class BatbEventsController extends LocalAppController {
	...
	public function add() {
		...
		if(count($this->data)>0) {
			...
			if($this->BatbEvent->save($this->data)) {
				
				$event = $this->BatbEvent->read();
				
				$this->BatbEvent->BatbMatch->createMatches($event);
				
				$this->Session->setFlash("Successfully Created Event: ".$this->data['BatbEvent']['name']);
	...
	public function update_match_winners() {
		...
		$this->BatbMatch->updateWinners($this->data);
		...
		$this->BatbVote->updateAll(
			...
			array("BatbVote.batb_match_id"=>$this->data['BatbMatch']['id'],"BatbVote.rps_winner_user_id"=>$this->data['BatbMatch']['rps_winner_user_id'])
		
		);
		//update the rps match points
		$this->BatbScore->query(
			"UPDATE batb_scores SET rps_score = rps_score+1
			WHERE user_id IN (SELECT user_id FROM batb_votes WHERE batb_match_id='".$this->data['BatbMatch']['id']."'
			AND rps_result=1)"
		);
		...
		$this->BatbVote->updateAll(
			...
			array("BatbVote.batb_match_id"=>$this->data['BatbMatch']['id'],"BatbVote.match_winner_user_id"=>$this->data['BatbMatch']['match_winner_user_id'])
		);
		...
		$this->BatbVote->updateAll(
			...
			array("BatbVote.batb_match_id"=>$this->data['BatbMatch']['id'],"BatbVote.winner_letters"=>$this->data['BatbMatch']['winner_letters'],"BatbVote.match_result"=>1)
		);
		...
		$this->BatbVote->query(
			"UPDATE batb_votes SET total_points=(rps_points+match_points+letters_points) WHERE batb_match_id='".$this->data['BatbMatch']['id']."'"
		);
		
		//update all the votes to processed
		$this->BatbVote->updateAll(
			array("BatbVote.processed"=>1),
			array("BatbVote.batb_match_id"=>$this->data['BatbMatch']['id'])
		);
		...
		//$this->redirect(array("action"=>"view",$this->data['BatbMatch']['batb_event_id']));
