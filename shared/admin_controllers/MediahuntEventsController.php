...
class MediahuntEventsController extends LocalAppController {
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid mediahunt event'));
	...
	function add() {
		if (!empty($this->request->data)) {
			...
			if ($this->MediahuntEvent->save($this->request->data)) {
				$this->Session->setFlash(__('The mediahunt event has been saved'));
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('The mediahunt event could not be saved. Please, try again.'));
			}
		}
	}
	...
	function edit($id = null) {
		...
		if (empty($this->request->data)) {
			$this->request->data = $this->MediahuntEvent->read(null, $id);
		}
	}

	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for mediahunt event'));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->MediahuntEvent->delete($id)) {
			$this->Session->setFlash(__('Mediahunt event deleted'));
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Mediahunt event was not deleted'));
		$this->redirect(array('action' => 'index'));
	}
