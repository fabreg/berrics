...
596,ZANESVILLE';
	...
	public function checkout_shipments() {
		...
		$ids = array(10720640,11115075,11121337,11421323,11650649,12008611,12112417,12337075,13826342,14062626,14120166,14350399,15696966,16663151,16881438,17437040,18043895,18424888,18813554,19404200,20120621,20512417,20977995,21326318,21623731,21892078,22915708,23192262,23298281,23427687,23495298,23790021,23976997,24842122,24913223,25597328,25728265,26514282,26914998,26916742,27466353,27599152,27879926,28243351,28468354,28934091,29427759,29549116,29774927,30034606,30591198,30711949,31004854,31022554,31209380,31471809,31511939,33051905,33146814,33197577,34232460,34258822,35400305,35909894,36357297,36534091,36766039,37366489,37473113,37987421,38273555,38479564,38792196,38882007,38910528,39093828,39627977,40284114,41352954,41664423,41770865,41864232,42160201,42277881,42460786,42776036,43004599,43029835,43285404,43454510,45617286,46006322,46362927,46575811,46619108,48019834,48285261,48694004,48890921,49672041,49722948,49887948,50427924,50748833,51336019,52674658,52719724,52949767,53175498,53819457,54291523,55016929,55064415,55285316,55357872,55638461,55904534,56189718,57315639,57529163,58750559,60335042,61011703,61848759,62159277,62207116,62851506,64064784,66518207,66751444,67476108,67628478,68540573,68553604,69261217,69283480,69776719,69880252,70024445,70317742,70454266,70633886,71120712,71230177,71423539,71644754,72148233,72166687,72553904,74674269,75690927,75732857,76075743,76646570,77262406,77548162,77874623,79029364,79262882,79575967,79591416,79947107,79956565,80670429,80932166,82067622,82103532,83428612,84018945,84464350,85561231,86741051,87014080,87734319,87847001,88381830,88502339,88637009,88758470,89041769,89621469,89787546,90112772,90232619,90781239,91021745,91287382,91563690,91580189,91691835,91721646,91825594,92019342,92094676,92375026,94760354,95044992,96431177,96439819,96962726,97161486,97421656,97943328,98421049,98697338);
	...
	public function batbox() {
		...
		$ids = $this->CanteenProduct->find('all',array(
					"conditions"=>array(
						"CanteenProduct.parent_canteen_product_id"=>1001919
					),
					"contain"=>array(),
					"fields"=>array("CanteenProduct.id")
				));

		$ids = Set::extract("/CanteenProduct/id",$ids);

		$orders = $this->CanteenOrder->CanteenOrderItem->find('all',array(
						"conditions"=>array(
							"CanteenOrderItem.canteen_product_id"=>$ids
						),
						"contain"=>array(
							"ParentCanteenOrderItem"
						),
						"fields"=>array("ParentCanteenOrderItem.canteen_order_id")
					));

		$order_ids = Set::extract("/ParentCanteenOrderItem/canteen_order_id",$orders);

		$approved_orders = $this->CanteenOrder->find('all',array(
								"conditions"=>array(
									"CanteenOrder.id"=>$order_ids,
									"CanteenOrder.order_status"=>"approved"
								),
								"contain"=>array(),
								"fields"=>array("CanteenOrder.id")
							));

		$approved_orders = Set::extract("/CanteenOrder/id",$approved_orders);

		$shipments = $this->CanteenOrder->CanteenShippingRecord->find('all',array(
							"conditions"=>array(
								"CanteenShippingRecord.canteen_order_id"=>$approved_orders
							),
							"contain"=>array(
								"UserAddress"
							)
	...
	public function ship_emails() {
		
		$this->loadModel('CanteenShippingRecord');
		
		$e = $this->CanteenShippingRecord->find('all',array(
					"conditions"=>array(
						"CanteenShippingRecord.id"=>array(10720640,11115075,11421323,12112417,13673979,13826342,16245347,18424888,18813554,21326318,21623731,23019218,23790021,24913223,24989055,27599152,27879926,28243351,28468354,28934091,29774927,30711949,31209380,33051905,33146814,35909894,37987421,38479564,40284114,41352954,41664423,41770865,42160201,42277881,42460786,43908103,45617286,46362927,46575811,47434249,48694004,48890921,49887948,52719724,52907612,53819457,55064415,61438732,64064784,68553604,70454266,71120712,71230177,72148233,72166687,72553904,79575967,79591416,79947107,79956565,81835162,84018945,86741051,87014080,87847001,88758470,90112772,90232619,91287382,91563690,91721646,94685459,96431177,96439819,96962726)
					),
					"contain"=>array("UserAddress")
				));

		$emails = Set::extract("/UserAddress/email",$e);
