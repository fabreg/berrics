...
class TrafficReportsController extends LocalAppController {
	...
	public function monthly() {
		...
		if(isset($this->request->data['Filters']['report_month']['month'])) {
			
			$month = $this->request->data['Filters']['report_month']['month'];
			
		}
		
		if(!$month && isset($this->request->params['named']['report_month'])) {
			
			$month = $this->request->data['Filters']['report_month']['month'] = $this->request->params['named']['report_month']['month'];
		...
		if(!$year && Set::check($this->request->params,"named.report_year")) {
			
			
			$year = $this->request->data['Filters']['report_year']['year'] = $this->request->params['named']['report_year'];
		...
		if(!$domain_id && Set::check($this->request->params,"named.dim_domain_id")) {
			
			
			$domain_id = $this->request->data['Filters']['dim_domain_id'] = $this->request->params['named']['dim_domain_id'];
			
		}
		
		//get the country
		
		if(Set::check($this->request->data,"Filters.country_code")) {
			
			$country_code = $this->request->data['Filters']['country_code'];
			
		}
		
		if(!$country_code && Set::check($this->request->params,"named.country_code")) {
			
			$country_code = $this->request->params['named']['country_code'];
	...
	public function day($date = false) {
		...
		//we got the date
		if(!isset($this->request->data['Filters']['report_date'])) {
			
			$this->request->data['Filters']['report_date'] = $date;
			
		} else {
			
			$date = $this->request->data['Filters']['report_date'];
			
		}

		//check for a domain
		if(isset($this->request->params['named']['dim_domain_id'])) {
			
			$this->request->data['Filters']['dim_domain_id'] = $this->request->params['named']['dim_domain_id'];
		...
		if(isset($this->request->data['Filters']['dim_domain_id']) && !empty($this->request->data['Filters']['dim_domain_id'])) {
			
			$sql_filter = " AND FactPageView.dim_domain_id = '".$this->request->data['Filters']['dim_domain_id']."'";
	...
	public function country_month_index() {
		...
		if(isset($this->request->data['Filters']['report_year']['year'])) {
		...
		$this->request->data['Filters']['report_year']['year'] = $year;
		$this->request->data['Filters']['report_month']['month'] = $month;
		
		$domain_id = false;
		
		if(isset($this->request->data['Filters']['dim_domain_id'])) {
			
			$domain_id = $this->request->data['Filters']['dim_domain_id'];
			
		}
		
		if(isset($this->request->params['named']['dim_domain_id']) && !$domain_id) {
	...
	public function country_month_overview() {
		...
		if(!$month && isset($this->request->params['named']['report_month'])) {
			...
			$month = $this->request->data['Filters']['report_month']['month'] =  $this->request->params['named']['report_month'];
		...
		if(!$year && isset($this->request->params['named']['report_year'])) {
		...
		if(!$domain_id && isset($this->request->params['named']['dim_domain_id'])) {
		
			$domain_id = $this->request->data['Filters']['dim_domain_id'] = $this->request->params['named']['dim_domain_id'];
			
		}
		
		if(isset($this->request->data['Filters']['country_code'])) {
			
			$country_code = $this->request->data['Filters']['country_code'];
			
		}
		
		if(!$country_code && isset($this->request->params['named']['country_code'])) {
			
			$country_code = $this->request->data['Filters']['country_code'] = $this->request->params['named']['country_code'];
	...
	public function country_by_day($date = false,$country_code = false) {
		...
		if(Set::check($this->request->data,"Filters.report_date")) {
		...
		if(Set::check($this->request->data,"Filters.country_code")) {
			...
			$this->request->data['Filters']['country_code'] = $country_code;
		...
		if(isset($this->request->data['Filters']['dim_domain_id'])) {
			...
			$dim_domain_id = $this->request->data['Filters']['dim_domain_id'];
			
		}
		
		if(isset($this->request->params['named']['dim_domain_id']) && !$dim_domain_id) {
			
			$this->request->data['Filters']['dim_domain_id'] = $dim_domain_id = $this->request->params['named']['dim_domain_id'];
	...
	public function media_files() {
		...
		if(count($this->request->data)>0) {
			...
			foreach($this->request->data['FactMediaView'] as $k=>$v) {
		...
		if(!isset($this->request->params['named']['date_start'])) {
			...
			$this->request->params['named']['date_start'] = date("Y-m-d",strtotime("-10 Day"));
			
		}
		if(!isset($this->request->params['named']['date_end'])) {
			
			$this->request->params['named']['date_end'] = date("Y-m-d");
			
		} 
		if(!isset($this->request->params['named']['limit'])) {
			
			$this->request->params['named']['limit'] = 20;
		...
		if(isset($this->request->params['named']['media_file_id'])) {
			...
			$sql .=" AND FactMediaView.media_file_id = '{$this->request->params['named']['media_file_id']}'";
		...
		$report = $this->FactMediaView->query("
			...
			WHERE DimDate.report_date between '{$this->request->params['named']['date_start']}' AND '{$this->request->params['named']['date_end']}' 
			{$sql}
	...
	public function media_file_details() {
		...
		if(!isset($this->request->params['named']['date_start'])) {
			...
			$this->request->params['named']['date_start'] = date("Y-m-d",strtotime("-10 Days"));
		...
		if(preg_match('/^(queue)/',$this->request->params['named']['media_file_id'])) {
	...
	public function media_monthly_overview() {
		...
		if(isset($this->request->data['MediaViewFilters'])) {
			
			return $this->redirect($this->request->data['MediaViewFilters']);
		...
		if(!isset($this->request->params['named']['date_start']) || !isset($this->request->params['named']['date_end'])) {
			
			$this->request->params['named']['date_start'] = date("Y-m-d",strtotime("-7 Days"));
			$this->request->params['named']['date_end'] = date("Y-m-d");
	...
	public function media_daily($date_in = false) {
		...
		if(isset($this->request->data['MediaReportDaily']['report_date'])) {
			
			return $this->redirect(array(0=>$this->request->data['MediaReportDaily']['report_date']));
