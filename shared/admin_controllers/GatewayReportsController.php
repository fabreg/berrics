...
class GatewayReportsController extends LocalAppController {
	...
	public function view() {
		...
		if(
			empty($this->request->data['GatewayAccount']['id']) ||  
			empty($this->request->data['GatewayAccount']['date_start']) || 
			empty($this->request->data['GatewayAccount']['date_end'])
		) {
		...
		$account = $this->GatewayAccount->find("first",array(
			...
			"conditions"=>array(
				"GatewayAccount.id"=>$this->request->data['GatewayAccount']['id']
			),
			"contain"=>array()
		...
		$report = $this->GatewayTransaction->query(
			"SELECT SUM(GatewayTransaction.amount) AS `total`,GatewayTransaction.method
				...
				WHERE GatewayTransaction.gateway_account_id='{$this->request->data['GatewayAccount']['id']}'
				AND GatewayTransaction.created BETWEEN '{$this->request->data['GatewayAccount']['date_start']}' 
				AND '{$this->request->data['GatewayAccount']['date_end']}'
