...
class SlsController extends LocalAppController {
	...
	public function add_entry() {
		...
		if(count($this->request->data)>0) {
			...
			if($this->SlsEntry->save($this->request->data)) {
	...
	public function edit_entry($id = false) {
		...
		if(count($this->request->data)>0) {
			...
			$this->SlsEntry->id = $this->request->data['SlsEntry']['id'];
			
			$this->SlsEntry->save($this->request->data);
			...
			$this->request->data = $this->SlsEntry->find("first",array(
				"conditions"=>array(
					"SlsEntry.id"=>$id
				),
