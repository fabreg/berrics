...
class YounitedNationsEventsController extends LocalAppController {
	...
	public function add() {
		...
		if(count($this->request->data)>0) {
			...
			if($this->YounitedNationsEvent->save($this->request->data)) {
	...
	public function view($id = false) {
		...
		if(isset($this->request->params['named']['s'])) {
			...
			if(isset($this->request->params['named']['YounitedNationsPosse.name'])) {
				
				
				$this->request->data['YounitedNationsPosse'] = $this->request->params['named']['YounitedNationsPosse.name'];
				
				$this->paginate['YounitedNationsEventEntry']['conditions']['YounitedNationsPosse.name LIKE'] = "%".str_replace(" ","%",$this->request->params['named']['YounitedNationsPosse.name'])."%";
	...
	public function view_entry($id = false) {
		...
		if(count($this->request->data)>0) {
			...
			$this->YounitedNationsEventEntry->id = $this->request->data['YounitedNationsEvenEntry']['id'];
			
			$this->YounitedNationsEventEntry->save($this->request->data);
