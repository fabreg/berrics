			...
			<?php 
				...
				//are we in the top bracket? If so the let's check to see if there has been a player selected
					if($top_bracket) {
						...
						$player1_html = "<div><div class='Player1UserId".$match['BatbMatch']['id']."'>{$player1_name}</div><a href=\"javascript:UserSearch.openSearch('userSelected','Player1UserId".$match['BatbMatch']['id']."')\">Click Here to Select Player 1</a>".$this->Form->input("BatbMatch.player1_user_id",array("type"=>"hidden","id"=>"Player1UserId".$match['BatbMatch']['id'],"value"=>$match['BatbMatch']['player1_user_id']))."</div>";
						$player2_html = "<div><div class='Player2UserId".$match['BatbMatch']['id']."'>{$player2_name}</div><a href=\"javascript:UserSearch.openSearch('userSelected','Player2UserId".$match['BatbMatch']['id']."')\">Click Here to Select Player 2</a>".$this->Form->input("BatbMatch.player2_user_id",array("type"=>"hidden","id"=>"Player2UserId".$match['BatbMatch']['id'],"value"=>$match['BatbMatch']['player2_user_id']))."</div>";
