...
$charges = Set::extract("/GatewayTransaction[method=/charge|capture/i][approved=1]",$this->request->data);

$charge_list = array();

foreach($charges as $c) $charge_list[$c['GatewayTransaction']['id']] = $c['GatewayTransaction']['method']." ".$c['GatewayTransaction']['amount']; 

$accs = Set::extract("/User/UserBillingProfile",$this->request->data);
...
foreach($this->request->data['GatewayTransaction'] as $t) if($t['approved'] == 1) @$trans_totals[$t['method']] += $t['amount'];
...
<script>
var order_json = <?php echo json_encode($this->request->data); ?>;
...
<div class='index form'>
	...
	<?php 
		echo $this->Form->create("CanteenOrder",array("url"=>$this->request->here,"id"=>"edit-form"));
	...
	<div>
		...
		<div>
			<h1>Edit Order</h1>
			<a href='<?php echo $this->request->here; ?>'>Refresh Order</a> | <a href='<?php echo (isset($this->request->params['pass'][1])) ? base64_decode($this->request->params['pass'][1]):'/canteen_orders/'; ?>'>Back to order manager</a>
			 | <a href='/canteen_orders/print_invoice/<?php echo $this->request->data['CanteenOrder']['id']; ?>' target='_blank'>Print Packing List</a> | 
			<?php 
								
								switch(strtoupper($this->request->data['CanteenOrder']['order_status'])) {
									
									case "AUTHORIZED":
										
										echo "<a extra='' func='CaptureOrder' class='func-link' confirm='' href='#'>Capture Order</a> | ";
										//echo $this->Form->submit("Capture and Approve Order",array("name"=>"data[CaptureOrder][{$this->request->data['CanteenOrder']['id']}]"));
									break;
								}
								...
								//echo $this->Form->submit("Cancel Order",array("name"=>"data[CancelOrder][{$this->request->data['CanteenOrder']['id']}]"));
			...
			<legend>Order Info</legend>
				...
				<table cellspacing='0'>
					<tr>
						...
						<td><?php echo strtoupper($this->request->data['CanteenOrder']['id']); ?></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><?php echo strtoupper($this->request->data['CanteenOrder']['wh_status']); ?></td>
					</tr>
					<tr>
						<td>Shipping Status:</td>
						<td><?php echo strtoupper($this->request->data['CanteenOrder']['shipping_status']); ?></td>
					</tr>
					<tr>
						<td>Shipping Carrier:</td>
						<td><?php echo strtoupper($this->request->data['CanteenOrder']['shipping_carrier']); ?></td>
					</tr>
					<tr>
						<td>Shipping Carrier Tracking:</td>
						<td><?php echo strtoupper($this->request->data['CanteenOrder']['shipping_carrier_tracking']); ?></td>
					</tr>
					<tr>
						<td>GEO IP Country:</td>
						<td><?php echo strtoupper($this->request->data['CanteenOrder']['geoip_country_code']); ?> (<?php echo strtoupper($this->request->data['CanteenOrder']['ip_address']); ?>)</td>
					</tr>
					...
					<tr>
						...
						<td>
							...
							<?php echo $this->request->data['Currency']['name']; ?> (<?php echo $this->request->data['Currency']['id']; ?>)
							<?php 
					...
					<tr>
						<td>Sub-Total:</td>
						<td><?php echo $this->Form->text("CanteenOrder.sub_total"); ?> (<?php echo $this->request->data['CanteenOrder']['currency_id']; ?>) </td>
					</tr>
					<tr>
						<td>Tax Total:</td>
						<td><?php echo $this->Form->text("CanteenOrder.tax_total"); ?> (<?php echo $this->request->data['CanteenOrder']['currency_id']; ?>)</td>
					</tr>
					<tr>
						<td>Shipping Total:</td>
						<td><?php echo $this->Form->text("CanteenOrder.shipping_total"); ?> (<?php echo $this->request->data['CanteenOrder']['currency_id']; ?>)</td>
					</tr>
					<tr>
						<td>Order Total:</td>
						<td><?php echo $this->Form->text("CanteenOrder.total"); ?> (<?php echo $this->request->data['CanteenOrder']['currency_id']; ?>)</td>
					</tr>
				</table>
				</div>
			...
			<fieldset id='credit-debit-forms'>
				...
				<legend>Credit / Debit</legend>
					<div style='padding:5px; background-color:#e9e9e9; font-size:12px; text-align:center;'>
						* Values Entered Will Be Applied In <em>"<?php echo $this->request->data['Currency']['name']; ?> (<?php echo $this->request->data['Currency']['id']; ?>) Rate: <?php echo $this->request->data['Currency']['rate']; ?>"</em>
					</div>
					<div style='width:48%; float:left;' id='credit-form'>
						...
						<div style='padding-top:8px; padding-bottom:8px;'>
							<strong>Total Credit:</strong> <span id='total-credit-div'>0.00</span> (<?php echo $this->request->data['Currency']['id']; ?>)
						</div>
						<?php 
					...
					<div style='width:48%; float:right;' id='debit-form'>
						...
						?>
						<div style='padding-top:8px; padding-bottom:8px;'>
							<strong>Total Debit:</strong> <span id='total-debit-div'>0.00</span> (<?php echo $this->request->data['Currency']['id']; ?>)
						</div>
						<?php 
		...
		<fieldset>
			...
			<table cellspacing='0'  class='line-items'>
				...
				<?php 
					foreach($this->request->data['CanteenOrderItem'] as $k=>$i):
				?>
				<tr <?php foreach($i as $key=>$val) if(!is_array($val)) echo "{$key}='{$val}'"; ?> class='order-item-row'>
					...
					<td align='center'><?php echo $this->Form->input("CanteenOrderItem.{$k}.sales_tax",array("label"=>"({$this->request->data['CanteenOrder']['currency_id']})")); ?></td>
					<td align='center'><?php echo $this->Form->input("CanteenOrderItem.{$k}.price",array("label"=>"({$this->request->data['CanteenOrder']['currency_id']})")); ?></td>
					<td align='center'><?php 
		...
		<fieldset>
			...
			<table cellspacing='0'>
				...
				<?php 
					foreach($this->request->data['EmailMessage'] as $e):
				...
				<tr>
					...
					<td class='actions'>
						<a href='/canteen_orders/resend_email/<?php echo $e['id']; ?>/<?php echo base64_encode($this->request->here); ?>'>Resend</a>
					</td>
				</tr>
				<?php 
					endforeach;
		...
		<fieldset>
			...
			<table cellspacing='0'>
				...
				<?php 
					...
					foreach($this->request->data['CanteenOrderNote'] as $k=>$n):
