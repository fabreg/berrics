...
<div class='index'>
	...
	<table cellspacing='0'>
		...
		<tr>
			<td>
				<?php if((strtoupper($s['shipping_status']) == "PENDING") || (isset($this->request->params['named']['CanteenShippingRecord.shipping_status']))): ?>
			...
			<td class='actions'>
				<a href='/canteen_shipping_records/edit/<?php echo $s['id']; ?>/callback:<?php echo base64_encode($this->request->here); ?>'>Edit</a>
				<?php if(strtoupper($s['shipping_status'])=="PENDING"): ?>
					<a href='/canteen_shipping_records/process_usps_shipment/<?php echo $s['id']; ?>/calback:<?php echo base64_encode($this->request->here); ?>'>USPS Shipment</a>
				<?php endif; ?>
				<?php if(strtoupper($s['shipping_status'])=="PROCESSING"): ?>
					<a href='/canteen_shipping_records/process_usps_shipment/<?php echo $s['id']; ?>/calback:<?php echo base64_encode($this->request->here); ?>' onclick='return confirm("Are you sure you want to regenerate this shipment");' >Regenerate USPS Shipment</a>
					<a href='/canteen_shipping_records/usps_combo_label/<?php echo $s['id']; ?>/calback:<?php echo base64_encode($this->request->here); ?>' >Print USPS Combo Label</a>
					<a href='/canteen_shipping_records/checkout_shipment/<?php echo $s['id']; ?>/<?php echo base64_encode($this->request->here); ?>'>Checkout Shipment</a>
					<a href="<?php echo $this->Html->url(array("action"=>"print_record",$s['id'])); ?>" class="btn btn-small" target="_blank">
