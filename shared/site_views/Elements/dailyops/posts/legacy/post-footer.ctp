...
<div class='footer'>
		<div class='tags'>
				...
				<?php 
					...
					if(count($t)>0) {
						...
						foreach($t as $v) {
							...
							if(isset($v['User']['facebook_account_num']) && !empty($v['User']['facebook_account_num'])) echo "<a href='http://facbook.com/profile.php?id={$v['User']['facebook_account_num']}' target='_blank' ><img border='0' src='/img/layout/fb-tag-icon.jpg'  /></a>";
						
							if(isset($v['User']['twitter_handle']) && !empty($v['User']['twitter_handle'])) echo "<a href='http://twitter.com/{$v['User']['twitter_handle']}' target='_blank' ><img border='0' src='/img/layout/twitter-tag-icon.jpg' /></a>";
							
							if(isset($v['User']['instagram_handle']) && !empty($v['User']['instagram_handle'])) echo "<a href='http://web.stagram.com/n/{$v['User']['instagram_handle']}' target='_blank' ><img border='0' src='/img/layout/instagram-tag-icon.jpg'  /></a>";
			...
			<div style='font-size:10px;'>
				<a href='http://cp.theberrics.com/dailyops/edit/<?php echo $d['id']; ?>' target='_blank'>Admin Edit</a> <a href='http://cp.theberrics.com/media_files/update_video_still/<?php echo $f['id']; ?>' target='_blank'>Update Video Still</a>
				<a href='http://cp.theberrics.com/traffic_reports/media_file_details/media_file_id:<?php echo $f['id']; ?>/date_start:2011-06-20/date_end:<?php echo date("Y-m-d"); ?>' target='_blank'>Media File Report</a> 
				
				<?php if(!in_array($f['id'],$report_queue)): ?>
				<a href='http://cp.theberrics.com/media_files/queue_video_for_report/<?php echo $f['id']; ?>/<?php echo base64_encode("http://".$_SERVER['SERVER_NAME'].$this->here)?>'>Queue for report</a>
				<?php 
					endif;
				?>
			</div>
